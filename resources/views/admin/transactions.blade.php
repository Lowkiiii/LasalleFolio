    ...
    <div class="flex flex-row justify-start">
        <div class="w-64 h-screen border-solid border-r-2">
            <ul>
                ...
                <li class="px-10 py-3.5 hover:bg-gray-200 transition ease-in-out duration-150 flex flex-row items-center">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 576 512" class="w-6"><!--!Font Awesome Free 6.5.1 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free Copyright 2024 Fonticons, Inc.--><path fill="#161513" d="M64 64C28.7 64 0 92.7 0 128V384c0 35.3 28.7 64 64 64H512c35.3 0 64-28.7 64-64V128c0-35.3-28.7-64-64-64H64zM272 192H496c8.8 0 16 7.2 16 16s-7.2 16-16 16H272c-8.8 0-16-7.2-16-16s7.2-16 16-16zM256 304c0-8.8 7.2-16 16-16H496c8.8 0 16 7.2 16 16s-7.2 16-16 16H272c-8.8 0-16-7.2-16-16zM164 152v13.9c7.5 1.2 14.6 2.9 21.1 4.7c10.7 2.8 17 13.8 14.2 24.5s-13.8 17-24.5 14.2c-11-2.9-21.6-5-31.2-5.2c-7.9-.1-16 1.8-21.5 5c-4.8 2.8-6.2 5.6-6.2 9.3c0 1.8 .1 3.5 5.3 6.7c6.3 3.8 15.5 6.7 28.3 10.5l.7 .2c11.2 3.4 25.6 7.7 37.1 15c12.9 8.1 24.3 21.3 24.6 41.6c.3 20.9-10.5 36.1-24.8 45c-7.2 4.5-15.2 7.3-23.2 9V360c0 11-9 20-20 20s-20-9-20-20V345.4c-10.3-2.2-20-5.5-28.2-8.4l0 0 0 0c-2.1-.7-4.1-1.4-6.1-2.1c-10.5-3.5-16.1-14.8-12.6-25.3s14.8-16.1 25.3-12.6c2.5 .8 4.9 1.7 7.2 2.4c13.6 4.6 24 8.1 35.1 8.5c8.6 .3 16.5-1.6 21.4-4.7c4.1-2.5 6-5.5 5.9-10.5c0-2.9-.8-5-5.9-8.2c-6.3-4-15.4-6.9-28-10.7l-1.7-.5c-10.9-3.3-24.6-7.4-35.6-14c-12.7-7.7-24.6-20.5-24.7-40.7c-.1-21.1 11.8-35.7 25.8-43.9c6.9-4.1 14.5-6.8 22.2-8.5V152c0-11 9-20 20-20s20 9 20 20z"/></svg>
                    <a href="/admin-fees" class="ml-5">Transactions Fees</a>
                </li>
            </ul>
        </div>
        <div class="w-[190rem] h-screen bg-neutral-100">
        <div class="m-7">
            <h2 class="text-3xl font-bold mb-7">User Transactions</h2>

            <table
                class="w-[100rem] text-sm font-light dark:border-neutral-500">
                    <thead class="font-medium text-black dark:border-neutral-500">
                        <tr>
                            ...
                            <th scope="col" class="text-left dark:border-neutral-500 pb-3.5">RECIPIENT</th>
                            <th scope="col" class="text-left dark:border-neutral-500 pb-3.5">TYPE</th>
                            <th scope="col" class="text-left dark:border-neutral-500 pb-3.5">AMOUNT</th>
                            <th scope="col" class="text-left dark:border-neutral-500 pb-3.5">BRANCH</th>
                            <th scope="col" class="text-left dark:border-neutral-500 pb-3.5">FEE (IN USD)</th>
                            <th scope="col" class="text-left dark:border-neutral-500 pb-3.5">DATE TIME</th>
                        </tr>
                    </thead>
                    <tbody class="bg-white">
                        @foreach ($trans as $cont)
                            <tr class="border-t border-b border-neutral-300 ease-in-out hover:bg-neutral-100">
                                <td class="flex flex-col whitespace-nowrap pl-4 py-4 dark:border-neutral-500">
                                    <div class="mb-0.5">
                                        {{ $cont->reference_number }}
                                    </div>
                                <td class="whitespace-nowrap py-4 dark:border-neutral-500">{{ $cont->sender_contact }}</td>
                                <td class="whitespace-nowrap py-4 dark:border-neutral-500">{{ $cont->recipient_contact }}</td>
                                <td class="whitespace-nowrap py-4 dark:border-neutral-500">{{ $cont->transaction_type }}</td>
                                <td class="whitespace-nowrap py-4 dark:border-neutral-500">{{ $cont->amount_local_currency }} {{ $cont->original_currency }}</td>
                                <td class="whitespace-nowrap py-4 dark:border-neutral-500">{{ $cont->branchSent->branch_code }} to {{$cont->branchReceived->branch_code}}</td>
                                <td class="whitespace-nowrap py-4 dark:border-neutral-500">{{ $cont->transactionFee->rates }}</td>
                                <td class="whitespace-nowrap py-4 dark:border-neutral-500">{{ $cont->datetime_transaction}}</td>
                                <td
                                    class="whitespace-nowrap px-6 py-4 dark:border-neutral-500 ">
                                    <form action="{{ route('admin.deletetransaction',$cont->id) }}" method="GET" onsubmit="return confirm('{{ trans('Are you sure you want to delete this ? ') }}');">
                                        @csrf
