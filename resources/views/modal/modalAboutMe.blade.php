<div class="hidden overflow-x-hidden overflow-y-auto fixed inset-0 z-50 outline-none focus:outline-none justify-center items-center animate-blink"
    ...
    <div class="relative w-1/2 px-4 my-auto mx-auto max-w-xl">
        <div
            ...
            <!-- Body -->
            <form action="{{ route('bio.update', $user->id) }}"
                  method="POST">
                @csrf
                @method('PUT') <!-- Use PUT method for updating -->
                <div class="relative pb-5 items-center justify-center flex-auto py-2">
                    <textarea name="bio"
                              class="bg-gray-200 p-3 text-black rounded-lg resize-none outline-none overflow-hidden w-full h-[10rem]"
                              placeholder="Tell us about yourself...">{{ old('bio', $bio->bio ?? '') }}</textarea>
                </div>
                <!-- Footer -->
...
<div class="hidden opacity-25 fixed inset-0 z-40 bg-black"
     id="modal-idAboutMe-backdrop"></div>
