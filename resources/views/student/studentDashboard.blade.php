                ...
                <div class="flex flex-row ">
                                            ...
                                            <div class="absolute right-0 top-0 z-20">
                                                ...
                                                <div class="editMenu hidden absolute bg-white space-x-2 py-2 px-5 border border-[#D4D4D4] rounded-lg shadow-lg w-55 transform translate-y-full bottom-5 left-1/2 -translate-x-1/2 z-10">
                                                    <ul class="text-sm font-semibold text-black w-auto">
                                                        ...
                                                        <div class="flex flex-row hover:text-[#FF0000]" onclick="toggleModal('modal-idPostDelete-{{ $post->id }}')">
                                                            <button type="submit" class="py-2">
                                                                <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="14" height="14" viewBox="0 0 30 30" class="fill-current opacity-80">
                    ...
                    <div class="flex flex-col w-2/5 ml-10 mr-10 z-20 animate-blink animation-delay-200">
                        <div
                            ...
                            <div class="flex-grow flex-col ml-4 text-lg font-bold text-black"> Suggested Students </div>
                            ...
                            <div class="flex mt-3">
                                    ...
                                    <div class="relative group">
                                        <label for="file_input" class="cursor-pointer">
                                            @if($student->profile_image)
                                                <img src="{{ $student->profile_image }}" alt="Profile" class="w-full h-full object-cover">
                                            @else
                                ...
                                <div class="flex-grow flex-col ml-4 text-sm font-bold text-black">
                                    <div class="flex">
                                        ...
                                        <div class="ml-1">
                                            <svg xmlns="http://www.w3.org/2000/svg" fill="#B7B7B7" height="18" width="18" version="1.1"
                                    ...
                                    <div class="text-xs font-semibold text-black opacity-45">
                                        Interests: {{ $student->interests->pluck('interest_name')->join(', ') }}
                                    </div>
                                </div>
                                <div class="flex items-center justify-center ml-5">
                                    <div class="text-xs font-bold">
                                        @php
                                            $friendRequest = \App\Models\FriendRequest::where(function($query) use ($student) {
                                        ...
                                        @if($friendRequest && $friendRequest->status === 'accepted')
                                            <form action="{{ route('friend-request.unfriend', $student->id) }}" method="POST">
                                                @csrf
                                                <button type="submit" class="inline-block rounded-md bg-[#FF0000] px-2 py-1 text-xs font-bold text-white">
                                                    Unfriend
                                                </button>
                                            </form>
                                        @elseif($friendRequest && $friendRequest->status === 'pending' && $friendRequest->receiver_id === auth()->id())
                                            <form action="{{ route('friendRequest.accept', $friendRequest->id) }}" method="POST" class="inline-block">
                                                @csrf
                                                <button type="submit" class="inline-block rounded-md bg-[#006634] px-2 py-1 text-xs font-bold text-white">
                                                    Accept
                                                </button>
                                            </form>
                                            <form action="{{ route('friendRequest.reject', $friendRequest->id) }}" method="POST" class="inline-block">
                                                @csrf
                                                <button type="submit" class="inline-block rounded-md bg-[#FF0000] px-2 py-1 text-xs font-bold text-white">
                                                    Reject
                                                </button>
                                        ...
                                        @elseif($friendRequest && $friendRequest->status === 'pending' && $friendRequest->sender_id === auth()->id())
                                            <button disabled class="inline-block rounded-md bg-gray-500 px-2 py-1 text-xs font-bold text-white">
                                                Pending
                                            </button>
                                        @else
                                            <form action="{{ route('friend-request.send', $student->id) }}" method="POST">
                                                @csrf
                                                <button type="submit" class="inline-block rounded-md bg-[#006634] px-2 py-1 text-xs font-bold text-white">
                                                    Connect +
                                                </button>
