                    ...
                    <div class="w-full mt-6 animate-blink animation-delay-200">
                        <form method="GET" action="{{ route('student.leaderboard') }}">
                            <div class="relative mb-6">
                                <input type="text" name="query" class="outline-none text-sm w-1/3 py-2 px-3 pl-10 border border-[#D4D4D4] rounded-2xl" placeholder="Search">
                                        ...
                                        <tr>
                                            <td class="py-4 text-[#006634] text-xl font-bold">
                                                <div class="items-center justify-center flex">
                                                    <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" fill="{{ $user->badge == 'Gold' ? '#FFD700' : ($user->badge == 'Silver' ? '#C0C0C0' : ($user->badge == 'Bronze' ? '#CD7F32' : '#000000')) }}" height="25" width="25" version="1.1" id="Capa_1" viewBox="0 0 296.084 296.084" xml:space="preserve" class="shadow-xl">
                                                        <g>
                                            ...
                                            <td class="py-4 font-bold text-black w-1/3">
                                                <div class="relative flex justify-center items-center text-center ">
                                                    <div class="w-12 h-12 rounded-full mx-5 flex mt-auto mb-auto overflow-hidden shadow-lg">
                                                        <div class="relative group">
                                                            <label for="file_input" class="cursor-pointer ">
                                                                @if ($user->image)
                                                                    <img src="{{ asset('storage/' . $user->image) }}" alt="{{ $user->name }}'s Profile Image" class="w-full h-full object-cover block" loading="lazy">
                                                                @else
                                                                    <img src="{{ asset('image/default-profile.png') }}" alt="Profile" class="w-full h-full object-cover block " loading="lazy">
                                                                @endif
                                                            </label>
                                                        </div>
                                                    </div>
                                                    ...
                                                    <div class="flex flex-col relative justify-start items-start ">
                                                        <div class="text-center font-bold text-black text-xl">{{ $user->name }}</div>
                                                        <div class="flex items-center text-xs font-bold text-black justify-center ">
                                                            {{-- <div class="flex items-center text-[#006634]">
                                                                {{ $user->connectedUsersCount }}
                                                                <div class="items-center ml-1 text-[#808080] text-xs font-semibold truncate">
                                                                    Connection |
                                                                </div>
                                                            </div>
                                                            <div class="flex items-center ml-1 text-[#006634]">
                                                                {{ $user->totalLikes }}
                                                                <div class="items-center ml-1 text-[#808080] text-xs font-semibold">
                                                                    Engagements
                                                                </div>
                                                            </div> --}}
                                                        </div>
